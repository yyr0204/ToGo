...
import test.spring.repository.map.PrintThread;
...
@Controller
@RequestMapping("/trip/*")
public class TripController extends Thread{
    ...
    @RequestMapping("stop")
    public @ResponseBody boolean stopPlace(){
        Thread thread = new TripController();
        thread.start();
        try {
            Thread.sleep(1000);
