	...
	<div class="container">
		...
		<table>
			<tr>
				<td class="right"><c:if test="${memId == null}">
						...
						<a class="btn btn-secondary" href="/ToGo/imageboard1/list">전체 글</a>
		...
		<c:if test="${count > 0}">
			<c:if test="${startPage > 10}">
				<a href="/ToGo/imageboard1/mylist?pageNum=${startPage - 10}">[이전]</a>
			</c:if>
			<c:forEach var="i" items="${page}">
				<a href="/ToGo/imageboard1/mylist?pageNum=${i}">[${i}]</a>
			</c:forEach>
			<c:if test="${endPage < pageCount}">
				<a href="/ToGo/imageboard1/mylist?pageNum=${startPage + 10}">[다음]</a>
			</c:if>
		</c:if>
	</div>
</body>
